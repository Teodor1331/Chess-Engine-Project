...
class GameState:
    ...
    def player_turn(self) -> bool:
        """Return the player turn property of the class."""
    ...
    def make_move(self, move) -> None:
        ...
        if self.board[move.start_row][move.start_col] != '.':
            ...
            self.__player_turn = not self.player_turn
            self.log_players_data()
    ...
    def log_players_data(self) -> None:
        """Show the game log for the players."""
        ...
        board_white_original = [
            'P', 'P', 'P', 'P', 'P', 'P', 'P', 'P',
            'R', 'N', 'B', 'Q', 'K', 'B', 'N', 'R'
        ]
        board_black_original = [
            'r', 'n', 'b', 'q', 'k', 'b', 'n', 'r',
            'p', 'p', 'p', 'p', 'p', 'p', 'p', 'p'
        ]
        board_white_existing = []
        board_black_existing = []

        for i in range(8):
            for j in range(8):
                if self.board[i][j] != '.':
                    if 'A' <= self.board[i][j] <= 'Z':
                        board_white_existing.append(self.board[i][j])
                    else:
                        board_black_existing.append(self.board[i][j])

        board_white_removed = list(
            (Counter(board_white_original) -
             Counter(board_white_existing))
            .elements()
        )
        board_black_removed = list(
            (Counter(board_black_original) -
             Counter(board_black_existing))
            .elements()
        )
        ...
        print('White Pieces taken: ', board_white_removed)
        print('Black Pieces taken: ', board_black_removed)
        print('===========\n')
        ...
        for direction in moving_directions:
            ...
            for i in range(1, 7):
                ...
                if 0 <= next_row_square < 8 and 0 <= next_column_square < 8:
                    ...
                    elif self.player_turn and self.board[next_row_square][next_column_square] in "rnbqp":
                        attack = Move([row, column], [next_row_square, next_column_square], self.board)
                        self.add_game_move(attack)
                    elif self.player_turn is False and self.board[next_row_square][next_column_square] in "RNBQP":
                        attack = Move([row, column], [next_row_square, next_column_square], self.board)
                        self.add_game_move(attack)
                    else:
                        break
